...
设len[i]为以该数结尾,能构成的最长序列的长度。进行连接的时候,对于每个数字num[i],遍历位置在它之前的数字num[j],如果比这个数小(num[j]<num[i]),也就是能
构成一个序列,这样就能进行状态转移,我们令len[i]=max(len[i],len[j]+1)来保证存储为最大长度,同时可以记录max(dp[i])

之后考虑数量,也就是说最长长度的序列有几个,设ans[i]为第i个数结尾的最长序列的个数,
状态转移的时候,如果len更新了,也就是说(len[j]+1>len[i])说明这个长度的序列是新出现的,需要将ans[i]设置为ans[j](用于初始化,第一次出现),因为
新序列中,最新的数提供了序列的尾巴,数量是由前面积累的(或者说转移),
例如[1 1 3 7]  当开始更新3的时候,一开始3的ans[2]=1,  而ans[0]=1         ans[0]+1>ans[2]   所以先初始化 ans[2]=1,len[2]=len[0]+1=2
